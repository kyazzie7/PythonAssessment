...
def display_menu(menu):
    ...
    if type(menu) is tuple:
        ...
        return -1
#2. Finding the number of elements in tuple using length
print("Answer for #2: " + str(len(menu)))

#Poor attempt to complete #3 through #7
options = input("Select a menu item by a number(1-6) and hit enter then done: ")
if options in menu:
        response=input("Thank you, we will get started. Please enter 'exit' if order is done: ")
elif response in menu:
        response = input("Thank you, we will get started. Please enter 'exit' if order is done: ")
else response =="exit" or response == "Exit":
        Exit() 
# ------ Place code above here /\ /\ /\ ------
...
def exercise09():
    ...
    #Not sure how to complete this BUT I know that I have to randomly pull 10 links so like a range[0,9] and then appead it to the dogs list
    #From there I would display the content since dog_media is linking the url.
...
def exercise10(sentence):
    ...
    # ------ Place code below here \/ \/ \/ ------
    #I'm unable to complete this successfully. But but here is my sudo code. 
    #1. I would take in the object , lets a word then use the slice method. 
    #2. Slice method would be starting at zero within the length of the word then stopped at -1. 
    #3. then return and print revert word 
    # ------ Place code above here /\ /\ /\ ------
    return reversed
